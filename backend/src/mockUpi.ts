...
const conceptCode = 'CASHFLOW_MEMORY_FOR_BHARAT';
...
export function buildMockUpiResponse(input: MockUpiRailRequest) {
  ...
  return {
    ...
    concept: 'Cashflow Memory for Bharat',
    txnId: input.txnId,
    ...
    npciStatus,
    responseCode,
    responseMessage: 'Cashflow Memory for Bharat sandbox response for ' + input.purpose + '. No real UPI rail, NPCI, bank, PSP, or customer data is used.',
    settlement: {

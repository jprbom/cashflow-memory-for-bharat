...
const CONFIG = {
  "title": "Cashflow Memory for Bharat",
  "short": "Explainable cashflow memory and credit-readiness engine for thin-file merchants.",
  "roles": [
    "ADMIN",
    "CREDIT_COACH",
    "LENDER_REVIEWER",
  ...
  "primary": {
    ...
    "createPayload": {
      "merchantName": "Demo Kirana Credit",
      "segment": "KIRANA",
      "city": "Pune",
      "consentStatus": "ACTIVE",
      "readinessScore": 704,
      "monthlyInflow": 98500,
      "monthlyOutflow": 64100
    },
    "patchPayload": {
      "readinessScore": 736,
      "monthlyInflow": 108900,
      "consentStatus": "ACTIVE"
    }
  },
  "secondary": {
    "label": "Cashflow Entries",
    "route": "/cashflow-entries"
  },
  "domain": {
...
export default function App() {
  ...
  function buildTabOutput(tab: WorkflowTab, summary: string, codes: string[] = []) {
    setTabOutput({
      ...
      facts: [
        ...
        CONFIG.secondary.label + ': ' + secondary.length + ' cashflow records',
  ...
  async function runActiveTabCta() {
    if (activeTab.apiFlow.includes(CONFIG.domain.endpoint)) {
      ...
      buildTabOutput(activeTab, 'Generated credit-readiness output from UPI cashflow test data.', ['READINESS_MODEL_READY', 'REASON_CODES_RENDERED']);
      return;
    }
    if (activeTab.apiFlow.includes('/mock-upi')) {
      await runMockRail(activeTab);
      buildTabOutput(activeTab, 'Sent a synthetic consent/payment rail request for this tab scenario.', ['MOCK_RAIL_RESPONSE', activeTab.mockScenario]);
      return;
    }
    if (activeTab.apiFlow.includes(CONFIG.primary.route)) {
      await createRecord();
      buildTabOutput(activeTab, 'Created a merchant profile from the tab-specific test payload.', ['CRUD_CREATE_OK', 'PROFILE_MEMORY_UPDATED']);
      return;
    }
    if (activeTab.apiFlow.includes(CONFIG.secondary.route)) {
      setSelected(secondary[0] ?? primary[0] ?? null);
      buildTabOutput(activeTab, 'Opened cashflow ledger evidence from live synthetic records.', ['CASHFLOW_ENTRY_READY', 'CONSENT_CONTEXT_ACTIVE']);
      return;
    }
    buildTabOutput(activeTab, 'Opened credit memory evidence from metrics, merchant profiles, and cashflow records.', ['TAB_CONTEXT_READY', 'TEST_DATA_BOUND']);
  }
      ...
      <main>
        ...
        <section className="workspace-grid">
          ...
          <div className="panel span-three">
            ...
            {paymentSimulation ? <PaymentTimeline result={paymentSimulation} /> : <p>Run Payment Ecosystem to simulate PG checkout, payment aggregator attempt, TPAP authorization, PSP/bank health, NPCI-style rail state, webhook delivery, and cashflow settlement memory.</p>}
          </div>
          <div className="panel">
